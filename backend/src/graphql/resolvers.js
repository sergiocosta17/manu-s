...
const resolvers = {
  ...
  Mutation: {
    updateProfile: async (_, args, { user }) => {
      if (!user) throw new Error('Não autenticado');
      return await User.findByIdAndUpdate(user.userId, { $set: args }, { returnDocument: 'after', runValidators: true });
    },
    ...
    updateProduct: async (_, { id, input }, { user }) => {
      if (!user || user.role !== 'ADMIN') throw new Error('Não autorizado');
      return await Product.findByIdAndUpdate(id, { $set: input }, { returnDocument: 'after', runValidators: true });
    },
    ...
    createOrder: async (_, { input }, { user }) => {
      if (!user) throw new Error('Não autenticado');
      const order = new Order({ ...input, user: user.userId, status: 'PENDING' });
      return await order.save();
    },
    updateOrderStatus: async (_, { id, status }, { user }) => {
      if (!user || user.role !== 'ADMIN') throw new Error('Não autorizado');
      return await Order.findByIdAndUpdate(id, { $set: { status } }, { returnDocument: 'after', runValidators: true });
    },
    signup: async (_, { name, email, password, role, adminKey }) => {
      const existingUser = await User.findOne({ email });
      if (existingUser) throw new Error('E-mail já cadastrado');
      if (role === 'ADMIN' && adminKey !== process.env.ADMIN_KEY) throw new Error('Chave de administrador inválida');
