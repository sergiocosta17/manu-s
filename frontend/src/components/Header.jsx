...
// Links de navegação para menu administrativo
const ADMIN_MENU_ITEMS = [
  { path: '/menu', label: 'Cardápio', icon: 'Home' },
  { path: '/promotions', label: 'Ofertas', icon: 'Tag' },
  { path: '/admin', label: 'Painel', icon: 'Chart' },
  { path: '/admin/products', label: 'Produtos', icon: 'Box' },
];
...
export default function Header() {
  ...
  // Detecta scroll para estilo do header
  useEffect(() => {
  ...
  // Mapeia ícones por nome (para evitar repetição)
                ...
                <>
                  ...
                  {isAdmin && (
                    <div className="relative" ref={adminMenuRef}>
                      ...
                      >
                        {isAdminMenuOpen ? <CloseIcon className="w-5 h-5" /> : <MenuHamburgerIcon className="w-5 h-5" />}
                      </button>
                  ...
                  {/* Acompanhar pedidos (cliente mobile) */}
                  {!isAdmin && (
                    <button
                      onClick={() => setIsTrackingOpen?.(true)}
